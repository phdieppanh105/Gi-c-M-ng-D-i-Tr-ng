...
export const AdminCharacterModal: React.FC<AdminCharacterModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (characterToEdit) {
        ...
        onSuccess({ id: characterToEdit.id, ...characterData });
      } else {
        const docRef = await addDoc(collection(db, 'characters'), characterData);
        onSuccess({ id: docRef.id, ...characterData });
      }
      onClose();
    } catch (error) {
      console.error('Error saving character:', error);
      setErrorMsg('Đã xảy ra lỗi khi lưu nhân vật vào cơ sở dữ liệu. Vui lòng thử lại.');
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 md:p-6 text-[#5d6d7e]">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 15 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 15 }}
          transition={{ duration: 0.25 }}
          className="relative w-full max-w-3xl bg-white rounded-[36px] border border-[#e1f5fe] shadow-2xl overflow-hidden my-auto max-h-[92vh] flex flex-col"
        >
          {/* Header */}
          <div className="sticky top-0 z-20 flex items-center justify-between px-6 py-4 bg-white/95 backdrop-blur-md border-b border-[#e1f5fe]">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-[#ffe0b2] text-[#fb8c00] flex items-center justify-center text-base font-bold shadow-2xs">
                ☾
              </div>
              <div>
                <h2 className="text-base sm:text-lg font-bold text-[#4a5568]">
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {/* Group 1: Basic Info */}
            <div className="space-y-4 bg-[#fffef0]/70 p-4 sm:p-5 rounded-3xl border border-[#ffe0b2]/50">
              <h3 className="text-xs font-bold uppercase tracking-wider text-[#4a5568] flex items-center gap-1.5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-2xl bg-white border border-[#e1f5fe] text-sm text-[#4a5568] placeholder-[#b0bec5] focus:outline-hidden focus:border-[#ffe0b2] focus:ring-2 focus:ring-[#ffe0b2]/30 transition-all"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-start">
                  <div className="w-24 h-24 rounded-2xl bg-white border-2 border-dashed border-[#e1f5fe] overflow-hidden flex items-center justify-center shrink-0 relative group">
                    {image ? (
                      <>
                        <img
                          src={image}
                      ...
                      <ImageIcon className="w-7 h-7 text-[#b0bec5]" />
                    )}
                  </div>
                  ...
                  <div className="flex-1 space-y-2 w-full">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#fce4ec] text-[#880e4f] hover:bg-[#f8bbd0] text-xs font-bold transition-colors cursor-pointer"
                      >
                        <Upload className="w-3.5 h-3.5" />
                        <span>{uploadingImage ? 'Đang nén...' : 'Tải ảnh từ thiết bị'}</span>
                      </button>
                    </div>
                    <input
                      type="url"
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-white border border-[#e1f5fe] text-xs text-[#4a5568] placeholder-[#b0bec5] focus:outline-hidden focus:border-[#ffe0b2]"
                    />
                  </div>
                </div>
              </div>
              ...
              {/* Slogan */}
              <div>
                <label className="block text-xs font-bold text-[#4a5568] mb-1 flex items-center gap-1">
                  <Quote className="w-3 h-3 text-[#ff8a65]" />
                  Slogan nhân vật
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-2.5 rounded-2xl bg-white border border-[#e1f5fe] text-sm text-[#4a5568] placeholder-[#b0bec5] focus:outline-hidden focus:border-[#ffe0b2] transition-all"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-[#4a5568] mb-1 flex items-center gap-1">
                  <Tag className="w-3 h-3 text-[#fb8c00]" />
            ...
            <div className="space-y-4 bg-slate-50/70 p-4 sm:p-5 rounded-3xl border border-[#e1f5fe]">
              <h3 className="text-xs font-bold uppercase tracking-wider text-[#4a5568] flex items-center gap-1.5">
                <BookOpen className="w-3.5 h-3.5 text-[#ff8a65]" />
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2.5 rounded-2xl bg-white border border-[#e1f5fe] text-xs sm:text-sm text-[#4a5568] placeholder-[#b0bec5] focus:outline-hidden focus:border-[#ffe0b2] transition-all"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-[#4a5568] mb-1 flex items-center gap-1">
                  <BookOpen className="w-3 h-3 text-[#880e4f]" />
                  Cốt Truyện
                </label>
                <textarea
                  value={story}
                  ...
                  className="w-full px-4 py-2.5 rounded-2xl bg-white border border-[#e1f5fe] text-xs sm:text-sm text-[#4a5568] placeholder-[#b0bec5] focus:outline-hidden focus:border-[#ffe0b2] transition-all font-serif"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-[#4a5568] mb-1 flex items-center gap-1">
                  <MessageSquare className="w-3 h-3 text-[#f48fb1]" />
              ...
              <div>
                <label className="block text-xs font-bold text-[#4a5568] mb-1 flex items-center gap-1">
                  <StickyNote className="w-3 h-3 text-[#ffb74d]" />
                  Ghi Chú Của Moon
                </label>
                <textarea
                  value={moonNote}
            ...
            <div className="space-y-4 bg-slate-50/70 p-4 sm:p-5 rounded-3xl border border-[#e1f5fe]">
              <h3 className="text-xs font-bold uppercase tracking-wider text-[#4a5568] flex items-center gap-1.5">
                <Bot className="w-3.5 h-3.5 text-[#0288d1]" />
                Liên Kết Ngoài
              </h3>
              ...
              <div>
                <label className="block text-xs font-bold text-[#4a5568] mb-1 flex items-center gap-1">
                  <Bot className="w-3 h-3 text-[#0288d1]" />
              ...
              <div>
                <label className="block text-xs font-bold text-[#4a5568] mb-1 flex items-center gap-1">
                  <HelpCircle className="w-3 h-3 text-[#c2185b]" />
            ...
            <div className="pt-2 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-[#fce4ec] hover:bg-[#f8bbd0] text-[#880e4f] text-xs font-bold shadow-sm hover:shadow-md disabled:opacity-50 transition-all cursor-pointer"
              >
                <Save className="w-4 h-4" />
                <span>{saving ? 'Đang lưu vào hệ thống...' : 'LƯU NHÂN VẬT'}</span>
              </button>
            </div>
          </form>
        </motion.div>
